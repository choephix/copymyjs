...
globalLoadingReasons.makeParentTo(userLoadingReasons, profileLoadingReasons);
...
2. **Clean Up Properly**
   ...
   class Component {
     ...
     init() {
       ...
       this.cleanup.push(this.reasons.add('initializing'));
     }
...
3. **Use with Async Operations**
   ```typescript
   async function loadData() {
     return reasons.addDuring('loadingData', fetch('/api/data'));
   }
   ```
